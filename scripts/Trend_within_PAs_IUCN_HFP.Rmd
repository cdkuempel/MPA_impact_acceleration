...
```{r}
chi <- list.files(here("data/hfp/projected/"), full=TRUE,
                  pattern="Proj")
...
```{r}
trend <- raster(here("data/trend/All_terrestrial_chi_slope.tif"))
```
...
for(i in 1:length(iucn)){
  ...
  writeRaster(wdpa_ras, here(paste0("data/wdpa/iucn/",iucn[i],"_hfp_raster.tif")), overwrite = T)
...
write.csv(all, here("data/paper/IUCN_hfp.csv"))
write.csv(all_trend, here("data/paper/IUCN_hfp_trend.csv"))
```
...
for(i in 2:length(strict)){
  ...
  writeRaster(wdpa_ras, here(paste0("data/wdpa/iucn/",strict[i],"_hfp_raster.tif")), overwrite = T)
...
```{r}
write.csv(all, here("data/paper/Strict_cat_IUCN_hfp.csv"))
write.csv(all_trend, here("data/paper/Strict_cat_IUCN_hfp_trend.csv"))
```
...
```{r}
all_wdpa_ras<-raster(here("data/wdpa/WDPA_All_terrestrial_raster.tif"))
wdpa_strict<-raster(here("data/wdpa/iucn/Strict_hfp_raster.tif"))
wdpa_not_strict<-raster(here("data/wdpa/iucn/Not_strict_hfp_raster.tif"))
wdpa_other<-raster(here("data/wdpa/iucn/Other_hfp_raster.tif"))
```
...
write.csv(all_chi_eez_df, here("data/paper/Country_hfp_results.csv"))

st_write(all_chi_eez, here("data/paper/Country_hfp_results.shp"), delete_layer = TRUE)
```
...
write.csv(all_chi_trend_eez_df, here("data/paper/Country_hfp_trend_results.csv"))

st_write(all_chi_trend_eez, here("data/paper/Country_hfp_trend_results.shp"), delete_layer = TRUE)
```
